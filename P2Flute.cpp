...
namespace stk {
    ...
    void P2Flute::setFrequency(StkFloat frequency)
    {
#if defined(_STK_DEBUG_)
        if (frequency <= 0.0) {
            oStream_ << "P2Flute::setFrequency: argument is less than or equal to zero!";
    ...
    void P2Flute::setJetDelay(StkFloat aRatio)
    {
        jetRatio_ = aRatio;
        jetDelay_.setDelay(boreDelay_.getDelay() * aRatio); // Scaled by ratio.
    }

    void P2Flute::startBlowing(StkFloat amplitude, StkFloat rate)
    {
        if (amplitude <= 0.0 || rate <= 0.0) {
            oStream_ << "P2Flute::startBlowing: one or more arguments is less than or equal to zero!";
            handleError(StkError::WARNING); return;
        }
    ...
    void P2Flute::stopBlowing(StkFloat rate)
    {
        if (rate < 0.0) {
            oStream_ << "P2Flute::stopBlowing: argument is less than zero!";
            handleError(StkError::WARNING); return;
        }
    ...
    void P2Flute::noteOn(StkFloat frequency, StkFloat amplitude)
    {
    ...
    void P2Flute::noteOff(StkFloat amplitude)
    {
        this->stopBlowing(amplitude * 0.02);
    }


    void P2Flute::controlChange(int number, StkFloat value)
    {
#if defined(_STK_DEBUG_)
        if (Stk::inRange(value, 0.0, 128.0) == false) {
            oStream_ << "P2Flute::controlChange: value (" << value << ") is out of range!";
            handleError(StkError::WARNING); return;
        }
#endif
        ...
        if (number == __SK_JetDelay_) // 2
            ...
            oStream_ << "P2Flute::controlChange: undefined control number (" << number << ")!";
